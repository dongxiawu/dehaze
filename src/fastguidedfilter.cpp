...
//    int Idepth = I.depth();
//
//    cv::Mat I_temp,p_temp;
//    cv::resize(I,I_temp,cv::Size(I.cols/scaleSize, I.rows/scaleSize));
//    cv::resize(p,p_temp,cv::Size(p.cols/scaleSize, p.rows/scaleSize));
//
//    int r_temp = r/scaleSize;
//
//    p_temp = convertTo(p_temp,Idepth);
//
//    cv::Mat result;
//
//    if (I.channels() == 1){
//        cv::Mat mean_I = boxfilter(I_temp, r_temp);
//        cv::Mat mean_II = boxfilter(I_temp.mul(I_temp), r_temp);
...
//
//        std::vector<cv::Mat> Ichannels;
//        cv::split(I_temp, Ichannels);
//
//        mean_I_r = boxfilter(Ichannels[0], r_temp);
//        mean_I_g = boxfilter(Ichannels[1], r_temp);
//        mean_I_b = boxfilter(Ichannels[2], r_temp);
//
//        // variance of I in each local patch: the matrix Sigma in Eqn (14).
//        // Note the variance in each local patch is a 3x3 symmetric matrix:
//        //           rr, rg, rb
//        //   Sigma = rg, gg, gb
//        //           rb, gb, bb
//        cv::Mat var_I_rr = boxfilter(Ichannels[0].mul(Ichannels[0]), r_temp) - mean_I_r.mul(mean_I_r) + eps;
//        cv::Mat var_I_rg = boxfilter(Ichannels[0].mul(Ichannels[1]), r_temp) - mean_I_r.mul(mean_I_g);
//        cv::Mat var_I_rb = boxfilter(Ichannels[0].mul(Ichannels[2]), r_temp) - mean_I_r.mul(mean_I_b);
//        cv::Mat var_I_gg = boxfilter(Ichannels[1].mul(Ichannels[1]), r_temp) - mean_I_g.mul(mean_I_g) + eps;
//        cv::Mat var_I_gb = boxfilter(Ichannels[1].mul(Ichannels[2]), r_temp) - mean_I_g.mul(mean_I_b);
//        cv::Mat var_I_bb = boxfilter(Ichannels[2].mul(Ichannels[2]), r_temp) - mean_I_b.mul(mean_I_b) + eps;
//
//        // Inverse of Sigma + eps * I
//        invrr = var_I_gg.mul(var_I_bb) - var_I_gb.mul(var_I_gb);
//        invrg = var_I_gb.mul(var_I_rb) - var_I_rg.mul(var_I_bb);
...
//        cv::Mat mean_p = boxfilter(p_temp, r_temp);
//
//        cv::Mat mean_Ip_r = boxfilter(Ichannels[0].mul(p_temp), r_temp);
//        cv::Mat mean_Ip_g = boxfilter(Ichannels[1].mul(p_temp), r_temp);
//        cv::Mat mean_Ip_b = boxfilter(Ichannels[2].mul(p_temp), r_temp);
//
//        // covariance of (I, p) in each local patch.
//        cv::Mat cov_Ip_r = mean_Ip_r - mean_I_r.mul(mean_p);
//        cv::Mat cov_Ip_g = mean_Ip_g - mean_I_g.mul(mean_p);
...
//        cv::Mat b = mean_p - a_r.mul(mean_I_r) - a_g.mul(mean_I_g) - a_b.mul(mean_I_b); // Eqn. (15) in the paper;
//
//        cv::resize(a_r,a_r,cv::Size(I.cols,I.rows));
//        cv::resize(a_g,a_g,cv::Size(I.cols,I.rows));
//        cv::resize(a_b,a_b,cv::Size(I.cols,I.rows));
//
//        cv::resize(b,b,cv::Size(I.cols,I.rows));
//
//
//        std::vector<cv::Mat> c;
//        cv::split(I, c);
//
//        return (boxfilter(a_r, r).mul(c[0])
//                + boxfilter(a_g, r).mul(c[1])
//                + boxfilter(a_b, r).mul(c[2])
//                + boxfilter(b, r));  // Eqn. (16) in the paper;
//    }
//
//    return convertTo(result,depth == -1 ? p.depth() : depth);
